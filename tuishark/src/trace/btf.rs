...
impl BtfParser {
        ...
        while pos + 12 <= type_end {
            ...
            if kind == BTF_KIND_STRUCT || kind == 5 {
                for _ in 0..vlen {
                    ...
                    let m_offset_raw = u32::from_le_bytes([data[pos + 8], data[pos + 9], data[pos + 10], data[pos + 11]]);
                    pos += 12;
                    // When kflag=1, offset field is split: bits 31:24 = bitfield size, bits 23:0 = bit offset
                    let bit_offset = if kflag { m_offset_raw & 0x00FFFFFF } else { m_offset_raw };
                    members.push(BtfRawMember { name_off: m_name, type_id: m_type, bit_offset });
                }
            } else {
                let extra = match kind {
                    ...
                    _ => {
                        if vlen > 0 && kind > 19 {
                            return Err(format!("unknown BTF kind {kind} with vlen={vlen}, cannot determine extra data size"));
                        }
                        0
                    }
                };
            ...
            types.push(BtfRawType { name_off, kind, kflag, size_type, members });
        }
    ...
    fn struct_members(&self, name: &str) -> Result<Vec<(String, usize)>, String> {
        for (idx, ty) in self.types.iter().enumerate() {
            if (ty.kind == BTF_KIND_STRUCT || ty.kind == 5) && self.get_string(ty.name_off) == name {
                ...
                self.collect_members(idx, 0, 0, &mut result);
                return Ok(result);
            }
        }
    ...
    fn collect_members(&self, type_idx: usize, base_bit_offset: u32, depth: usize, result: &mut Vec<(String, usize)>) {
        if depth > 32 {
            return;
        }
        ...
        for m in &ty.members {
            ...
            if field_name.is_empty() {
                if m.type_id > 0 && (m.type_id as usize - 1) < self.types.len() {
                    self.collect_members(m.type_id as usize - 1, abs_bit_offset, depth + 1, result);
                }
            } else {
                // bit_offset / 8 is correct for non-bitfield members; target fields
                // (head, dev, sk, transport_header, etc.) are never bitfields
...
mod tests {
    ...
    fn resolve_from_kernel_btf() {
        ...
        // transport_header should be 2 bytes before network_header
        assert_eq!(offsets.skb.network_header, offsets.skb.transport_header + 2);
        // head should be after network_header (pointer field, 8 bytes aligned)
        assert!(offsets.skb.head > offsets.skb.network_header);
    }
}
